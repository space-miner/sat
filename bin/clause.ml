...
module Clause = struct
  module T = struct
    ...
    let unit_literal_opt cls =
      if List.length cls = 1 then Some (List.hd_exn cls) else None
    ;;
        ...
        List.fold_left assignment ~init:false ~f:(fun acc (var, bool) ->
          let witness =
            ...
            then (
              ...
              return
                (if Bool.equal bool true
                 then Int.equal lit var
                 else Int.equal lit (neg var)))
            else false
          in
          acc || witness))
    ;;
